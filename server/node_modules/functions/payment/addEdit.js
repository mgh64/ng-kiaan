...
function add(payment, cb) {
  cb = cb || function() {};
  //add_payment
  get_paymentId(function(err, res) {
    if (err) {
      cb(null, {
        error: true,
        type: 'get_paymentId'
      ...
      payment.id = res;
      new db.operations(payment).save(function(err) {
        // error with db
        if (err) {
          cb({
            status: 400,
    ...
    if (data.owner_type == 'person') {
      ...
      if (data.cost && data.cost.cost_id && data.cost.price) {
        data.items.push({
          ...
          price: Number.NumtoEn(data.cost.price)
        });
      } else if (data.income && data.income.income_id && data.income.price) {
        data.items.push({
          income_id: data.income.income_id,
          price: Number.NumtoEn(data.income.price)
        });
      }
    } else if (data.owner_type == 'cost') {
      data.cost_id = data.owner_id;
      delete data.by_id;
      delete data.by_title;
    } else {
      cb({
